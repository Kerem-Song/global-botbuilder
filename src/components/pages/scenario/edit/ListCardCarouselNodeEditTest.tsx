...
export const ListCardCarouselNodeEditTest = () => {
  useNodeEditSave();
  const { t } = usePage();
                    ...
                    <div key={item.id}>
                      ...
                      <div className="m-b-8">
                        <Space direction="vertical">
                          <FormItem
                            error={
                              errors.view?.childrenViews?.[index]?.items?.[j]?.imageUrl
                            }
                          >
                            <Row align="center" gap={12} style={{ margin: 0 }}>
                              <Col span={7} className="itemProfileImg">
                                <ImageFileUploader
                                  imageCtrl={
                                    IMAGE_CTRL_TYPES.LIST_CAROUSEL_ITEM_IMAGE_CTRL
                                  }
                                  index={index}
                                  listItemIndex={j}
                                  isValid={
                                    errors.view?.childrenViews?.[index]?.items?.[j]
                                      ?.imageUrl
                                      ? false
                                      : true
                                  }
                      ...
                      <div className="m-b-8">
                        <Space direction="vertical">
                          <span className="label">
                            <FormItem
                              error={
                                errors.view?.childrenViews?.[index]?.items?.[j]?.title
                              }
                            >
                              <InputWithTitleCounter
                                label={t(`TITLE_INPUT`)}
                                ...
                                isLight={true}
                                {...register(
                                  `view.childrenViews.${index}.items.${j}.title`,
                                )}
                                textLength={
                                  watch(`view.childrenViews.${index}.items.${j}.title`)
                                    ?.length || 0
                                }
                                readOnly={isHistoryViewer}
                              />
                        ...
                        <Space direction="vertical">
                          <span className="label">
                            <InputWithTitleCounter
                              ...
                              isLight={true}
                              {...register(
                                `view.childrenViews.${index}.items.${j}.description`,
                              )}
                              textLength={
                                watch(
                                  `view.childrenViews.${index}.items.${j}.description`,
