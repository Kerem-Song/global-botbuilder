...
export const NodeLinkPopUpMenu = ({
  ...
  const [scenarioList, setScenarioList] = useState<
    {
      ...
      type: ItemType;
      data: {
        action: (name: string, id: string, start?: GuideInfo) => void;
        start?: GuideInfo;
      };
    }[]
  >();
  ...
  const handleMakingOtherFlow = (name: string, id: string, guide?: GuideInfo) => {
    console.log(id);
              ...
              {item.value === NODE_TYPES.OTHER_FLOW_REDIRECT_NODE && scenarioList ? (
                <div>
                  <Popper
                    ...
                    onChange={(m) => {
                      m.data?.action?.(m.name, m.id, m.data.start);
                    }}
                    popup
                    popupList
                  >
